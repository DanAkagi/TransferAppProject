...
public class Slave {
    ...
    private class StorageHandler extends Thread {
        ...
        private void handlePartitionDeletion(DataInputStream dis) throws IOException {
            ...
            Path dataDirectory = Paths.get(DATA_DIRECTORY);  // Répertoire contenant les fichiers
            if (!Files.isDirectory(dataDirectory)) {
                FileTransferLoggerSlave.log("Le répertoire de données n'existe pas : " + DATA_DIRECTORY);
                return;
            }
        
            try (DirectoryStream<Path> stream = Files.newDirectoryStream(dataDirectory, fileName + "*")) {
                ...
                for (Path file : stream) {
                    try {
                        Files.delete(file);
                        fileDeleted = true;
                        FileTransferLoggerSlave.log("Fichier supprimé : " + file.getFileName());
                    } catch (IOException e) {
                        FileTransferLoggerSlave.logError("Erreur lors de la suppression du fichier : " + file.getFileName(), e);
                    }
                }
        
                if (!fileDeleted) {
                    FileTransferLoggerSlave.log("Aucun fichier à supprimer pour : " + fileName);
                }
            } catch (IOException e) {
                FileTransferLoggerSlave.logError("Erreur lors de l'accès au répertoire de données", e);
            }
        }
